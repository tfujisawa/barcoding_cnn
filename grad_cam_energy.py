...
def grad_cam_energy(model, img_input, last_conv_layer, pred_layers):
...
if __name__ == "__main__":
    ...
    print (pred_class1)
    #exit()
    outfile = "grad.sqn.energy.{0}.txt".format(os.path.basename(sys.argv[1].rstrip("\/")))
    print(outfile)
    with open(outfile, "w") as f:
        for i in range(len(ealig)):
            g = grad_cam_energy(m, ealig[i:(i+1),:], lcl, prl)
